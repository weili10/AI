...
# TODO in V6:
#    attacker: base line definder -> astar defender (main goal: capsules)
#
...
class AstarAgent(CaptureAgent):
    ...
    def chooseAttackAction(self, gameState):
        ...
        # take more food
        if self.curFoodCarring == self.targetFood and self.targetFood < 4:
    ...
    def heuristicFoodDistance(self, gameState):
        ...
        if len(self.curGhosts) > 0:
            ...
            if ghostDists <= self.safeDistForEat:
                return len(self.curGhosts) * 999999
            else:
